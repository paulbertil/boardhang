...
// `detectSessionInUrl` (default true) auto-completes the Google OAuth redirect when the
// browser returns to the app; `persistSession` + `autoRefreshToken` (defaults) keep the
// session alive across reloads. We keep the defaults explicit for clarity.
