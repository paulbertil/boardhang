-- 0013_session_end_realtime.sql
-- Live session END, layered on the realtime substrate from 0011/0012. When an owner ends a
...
-- session:<id> channel gated by 0011's receive policy. The soft-delete leaves session_members
-- intact, so is_session_member() is still true at delivery time and members still receive it.
--
-- Design (mirrors 0011/0012):
...
-- 0011's session:<id> receive policy. Verify: when the owner ends a session, every other
