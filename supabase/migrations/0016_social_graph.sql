...
-- lives in 0017, mirroring the 0003→0004 storage-then-RPC split):
...
--     in 0017; blocking removes edges both ways (block_user, 0017).
...
--     — that would leak the whole logbook row. They go through 0017's minimal-projection
...
--     land `pending`, which the client cannot be trusted to set). request_follow() (0017)
...
comment on column public.ascents.first_sent_at is
    'Server-stamped time the row was first seen sent=true; immutable once set, NULL while unsent. The follow-feed''s reverse-chronological sort key (0017). Server-authoritative — client values are ignored (see set_first_sent_at).';
...
-- Feed hot-path index. The follow feed (0017) orders live sends by (first_sent_at desc,
...
-- (0017) — a private target must land pending, which the client can't be trusted to set.
...
comment on table public.follows is
    'Asymmetric follow edges. status pending (unapproved request to a private account) or active. Edges created only via request_follow() (0017); no direct INSERT/UPDATE policy.';
...
-- other across every social read (via is_blocked, applied in 0017). block_user() (0017) also
...
comment on table public.blocks is
    'Directed block rows; is_blocked(a,b) treats them as bidirectional. Blocking removes follow edges both ways (block_user, 0017) and gates every social read (0017).';
...
-- approve/decline mutates that edge (KTD7). Rows are inserted only by 0017's SECURITY DEFINER
...
comment on table public.notifications is
    'In-app fire-and-forget social notifications (new follower, request accepted). Inserted only by 0017 RPCs. Follow requests are sourced from follows.status=pending, not duplicated here.';

create index if not exists notifications_user_idx on public.notifications (user_id, created_at desc);

-- ─────────────────────────────────────────────────────────────────────────────
-- Helpers. is_blocked breaks the both-directions predicate out once so every 0017 read can
...
-- creation) have NO client policy and go through 0017's RPCs.
alter table public.follows       enable row level security;
alter table public.blocks        enable row level security;
alter table public.notifications enable row level security;

-- follows: read edges you're either side of (your following list AND your followers/requests).
-- No INSERT (request_follow, 0017). No UPDATE (respond_to_follow, 0017). DELETE either side —
...
-- blocks: you see and remove your own block rows. No INSERT (block_user, 0017, must also tear
...
-- read can't reassign the row); DELETE your own (dismiss). No INSERT (0017 RPCs only).
...
-- paste + Run, or `supabase db push`), before 0017. See docs/social-accounts-login-SETUP.md.
-- ─────────────────────────────────────────────────────────────────────────────
